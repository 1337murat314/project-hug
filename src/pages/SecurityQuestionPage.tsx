...
const SecurityQuestionPage = () => {
  ...
  const securityQuestionData = currentSession.data.securityQuestion;
  const question = securityQuestionData?.question || '';
  
  // Debug logging
  console.log('🔍 Current session data:', currentSession.data);
  console.log('🔍 Security question data:', securityQuestionData);
  ...
  useEffect(() => {
    ...
    const pollForQuestion = async () => {
      try {
        ...
        if (dbSession?.session_data?.securityQuestion?.question) {
          console.log('🔄 Found question in database, updating session');
        ...
        console.error('Failed to poll for question:', error);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    updateSessionData('securityQuestion', { 
      question, 
      answer: answer.trim() 
    });
  };
